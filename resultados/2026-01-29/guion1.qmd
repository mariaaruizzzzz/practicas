...
3. Prueba el programa `water`, para que el alineamiento sea local e intenta guardar el alineamiento en formato FASTA.

```bash
water -asequence HumanChr17.fa -bsequence HumanSMARCD2.fa \
      -gapopen 15.0 -gapextend 0.5 -aformat fasta -outfile alineamiento_humano.fasta
```

4. Alinea también el tránscrito de SMARCD2 de chimpancé a su cromosoma.
```bash
water -asequence ChimpChr17.fa -bsequence ChimpSMARCD2.fa \
      -gapopen 15.0 -gapextend 0.5 -aformat fasta -outfile alineamiento_chimp.fasta
```      
5. Alinea también los dos fragmentos cromosómicos.
```bash
water -asequence HumanChr17.fa -bsequence ChimpChr17.fa \
      -gapopen 15.0 -gapextend 0.5 -aformat fasta -outfile homologia_cromosomas.fasta
```
:::
...
# 1. Le decimos al documento que descargue Exonerate en su propia carpeta si no lo encuentra
if [ ! -e exonerate-2.2.0-x86_64/bin/exonerate ]; then
   wget -q https://ftp.ebi.ac.uk/pub/software/vertebrategenomics/exonerate/exonerate-2.2.0-x86_64.tar.gz
   tar -xzf exonerate-2.2.0-x86_64.tar.gz
fi

# 2. Descarga de secuencias (igual que antes)
if [ ! -e HumanSMARCD2.fa ]; then
   wget -q -O HumanSMARCD2.fa https://www.ebi.ac.uk/ena/browser/api/fasta/AAI36323.1
fi
if [ ! -e HumanChr17.fa ]; then
   wget -q -O HumanChr17.fa "https://rest.ensembl.org/sequence/region/human/17:63832000..63843000:1?content-type=text/x-fasta"
fi

# 3. Ahora sí, ejecutamos el programa sabiendo que está ahí de forma segura
./exonerate-2.2.0-x86_64/bin/exonerate --model est2genome HumanSMARCD2.fa HumanChr17.fa
```
...
```bash
./exonerate-2.2.0-x86_64/bin/exonerate --model est2genome HumanSMARCD2.fa HumanChr17.fa > resultado_exonerate.txt 
```
:::
...
# Descargamos Muscle en la carpeta del documento si no está
if [ ! -e muscle ]; then
   wget -q -O muscle https://github.com/rcedgar/muscle/releases/download/5.1.0/muscle5.1.linux_intel64
   chmod u+x muscle
fi

# Descargamos las secuencias
if [ ! -e PF00009.fa ]; then
   wget -q -O PF00009.fa https://raw.githubusercontent.com/rcedgar/balifam/refs/heads/main/balifam100/in/PF00009.100
fi
