...
# Solo clonamos si la carpeta no existe
if [ ! -d "hifiasm" ]; then
    echo "Descargando hifiasm..."
    git clone https://github.com/chhylp123/hifiasm
    cd hifiasm
    make
else
    echo "hifiasm ya está presente en ~/bin/hifiasm"
fi

# Añadimos al PATH para esta sesión
...
# Comprobamos si la carpeta final ya existe
if [ ! -d "compleasm_kit" ]; then
    echo "Descargando compleasm..."
    wget https://github.com/huangnengCSU/compleasm/releases/download/v0.2.7/compleasm-0.2.7_x64-linux.tar.bz2
    # El archivo de esta versión ya se descompone como compleasm_kit
    tar -jxvf compleasm-0.2.7_x64-linux.tar.bz2
    rm compleasm-0.2.7_x64-linux.tar.bz2
else
    echo "compleasm ya está listo en ~/bin/compleasm_kit"
fi
```
...
```
¿Cuál es la cobertura media estimada de las secuencias homocigotas? ¿Y la de las heterocigotas?
```bash
...
```{bash}
#| label: ejecucion-compleasm
#| eval: true

# Ejecutamos el análisis solo si no existe el archivo de resumen
...
```{bash}
#| label: visualizacion-calidad
#| eval: true

# Mostramos el contenido del resumen por pantalla para el informe
if [ -f asm1_eval/summary.txt ]; then
   cat asm1_eval/summary.txt
else
   echo "Esperando resultados... (Asegúrate de que el bloque anterior terminó con éxito)"
fi
```

```bash
"El análisis de completitud con compleasm muestra un 16.17% de genes Single-copy y un 83.51% de genes desaparecidos (Missing). Este valor de Missing tan elevado es esperado, ya que no estamos ensamblando el genoma completo de Arabidopsis thaliana, sino una fracción centrada principalmente en el Cromosoma 4. Por tanto, la ausencia de la mayoría de los ortólogos universales de las Brassicales es coherente con los datos de partida."
```
:::{.callout-note icon=false}
# Ejercicio 4
