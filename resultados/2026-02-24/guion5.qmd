...
En la segunda imagen, estás pasando la URL directamente como un texto plano. Como contiene espacios y comillas, la función curl_download se confunde o el servidor web rechaza la petición porque la sintaxis de la dirección es inválida. El error que ves (la línea roja en RStudio) es el lenguaje diciendo: "No entiendo esta dirección, tiene caracteres que no deberían estar ahí
...
fields <- 'fields=tax_id,scientific_name,last_updated,base_count,first_public&'
limit  <- 'limit=0'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'z2.txt')
```
