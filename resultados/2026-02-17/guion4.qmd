...
```bash
Suelen ser diferentes versiones de ensamblaje o niveles de completitud (ej. uno es "Reference" y otro "Representative").
```
...
```{r}
# Líneas repetidas
repetidos <- genomas[duplicated(genomas), ]

# Filas únicas
filas_unicas <- nrow(unique(genomas))
```
:::
...
```{r}
# Función para limpiar columnas fijas
limpiar_invariables <- function(df) {
  df[, sapply(df, function(x) length(unique(x))) > 1]
}

gene <- limpiar_invariables(gene)
cds  <- limpiar_invariables(cds)
...
resumen_completo <- rbind(
  resumen, # El de CDS que ya estaba creado arriba
...
:::{.callout-note icon=false}
## Desafío
Crea una tabla con el número total de genes en cada cromosoma.

¿Puedes contar cuántos hay codificados en cada sentido ("strand") y en cada cromosoma?

```{r}
# Número total de genes en cada cromosoma
table(gene$seqid)

# Número de genes codificados en cada sentido (+ o -) por cada cromosoma
table(gene$seqid, gene$strand)
```
:::
...
```{r}
# Añadimos la condición & gff$type == "gene" al filtro
...
```{r}
# Para que un gen esté parcialmente contenido, solo necesita solaparse.
# La condición es: el gen empieza antes de que acabe la región, 
# Y acaba después de que empiece la región.

f1_parcial <- gff$seqid == 'JAJSEB010000048.1' & gff$start <= 253478 & gff$end >= 184638
f2_parcial <- gff$seqid == 'JAJSEB010000012.1' & gff$start <= 348860 & gff$end >= 133444
f3_parcial <- gff$seqid == 'JAJSEB010000020.1' & gff$start <= 149614 & gff$end >= 73393
f4_parcial <- gff$seqid == 'JAJSEB010000016.1' & gff$start <= 272009 & gff$end >= 161247
f5_parcial <- gff$seqid == 'JAJSEB010000033.1' & gff$start <= 170687 & gff$end >= 123135

f_parcial <- f1_parcial | f2_parcial | f3_parcial | f4_parcial | f5_parcial
interesantes_parciales <- gff[f_parcial, ]
```
:::
...
```bash
La llave única de un archivo GFF se encuentra en la columna 9 ('attributes'), concretamente la etiqueta "ID=", ya que no puede haber dos elementos con el mismo ID.
```
:::
...
```{r}
# 1. Comprobar si "acc" es una llave perfecta (si sale 0, no hay duplicados)
anyDuplicated(genomas$acc)

# 2. Añadir base_count desde la tabla ENA a nuestra tabla genomas
genomas$base_count_ena <- genomas.ena[match(genomas$acc, genomas.ena$accession), "base_count"]

# 3. Comparar si hay diferencias gráficamente
plot(genomas$Assembly.Stats.ATGC.Count, genomas$base_count_ena, 
     xlab = "Longitud NCBI", ylab = "Longitud ENA", 
     main = "Comparación de tamaños de genomas")
```
:::
